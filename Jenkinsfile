pipeline {
    ...
    stages {
        ...
        stage('Provision new EC2 Instance') {
            steps {
                sh 'aws ec2 run-instances --region us-east-1 --image-id ${params.AMI_ID} --count 1 --instance-type ${params.EC2_INSTANCE_SIZE} --key-name ${params.EC2_KEY_NAME} --security-group-ids ${params.EC2_INSTANCE_SECURITY_GROUP} --subnet-id ${params.EC2_INSTANCE_SUBNET_ID} --tag-specifications \'ResourceType=instance,Tags=[{Key=Name,Value=${params.EC2_INSTANCE_NAME}}]\' ' 
